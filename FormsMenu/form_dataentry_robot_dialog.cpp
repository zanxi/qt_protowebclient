#include "form_dataentry_robot_dialog.h"
#include "ui_form_dataentry_robot_dialog.h"
...
form_dataentry_robot_Dialog::form_dataentry_robot_Dialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::form_dataentry_robot_Dialog)
{
    ui->setupUi(this);
    ...
    ui->groupBox_3->setStyleSheet("QGroupBox {"
                                  ...
                                  "background-color:"+DataSystems::Instance().settings___color_header+";"
                                                                                      "padding: 4 20000 4 10;"
                                                                                      "}");
    ui->groupBox_lot->setStyleSheet("QGroupBox {"
                                    "background-color: white;"
                                    "}"
                                    "QGroupBox::title {"
                                    "color: white;"
    ...
    ui->groupBox_tablewidget->setStyleSheet("QGroupBox {"
                                            "background-color: white;"
                                            "}"
                                            "QGroupBox::title {"
                                            "color: white;"
    ...
    /*
    {
        ...
        if(db_func->StatusOpen())
        {
            if(DataSystems::Instance().db_check_create)
            {
                ...
                db_func->sql_exec("DROP TABLE BigData;");
                db_func->sql_exec("CREATE TABLE IF NOT EXISTS  BigData"
    ...
    plot = new QCustomPlot(ui->groupBox_lot);
...
form_dataentry_robot_Dialog::~form_dataentry_robot_Dialog()
{
    //genbigdata->status = false;
    //if(mTimer->isActive())mTimer->stop();
    logger::WriteMsg("destructor form_dataentry_robot_Dialog::~form_dataentry_robot_Dialog()");
...
/*
void form_dataentry_robot_Dialog::setupAdvancedAxesDemo(QCustomPlot *customPlot)
{

//    QLayoutItem *child = ui->horizontalLayout_plot->takeAt(0);
...
//    delete w;
    ...
    //connect(customPlot->yAxis3, SIGNAL(rangeChanged(QCPRange)), customPlot->yAxis, SLOT(setRange(QCPRange))); // left axis only mirrors inner right axis
    ...
    customPlot->axisRect()->axis(QCPAxis::atLeft, 1)->setPadding(30); // add some padding to have space for tags
    //customPlot->axisRect()->axis(QCPAxis::atRight, 2)->setPadding(30); // add some padding to have space for tags

    // create graphs:
    mGraph1 = customPlot->addGraph(customPlot->xAxis, customPlot->axisRect()->axis(QCPAxis::atRight, 0));
    mGraph2 = customPlot->addGraph(customPlot->xAxis, customPlot->axisRect()->axis(QCPAxis::atLeft, 1));
    //mGraph3 = customPlot->addGraph(customPlot->xAxis, customPlot->axisRect()->axis(QCPAxis::atRight, 2));

    mGraph1->setPen(QPen(QColor(250, 120, 0)));
    mGraph2->setPen(QPen(QColor(0, 180, 60)));
    //mGraph3->setPen(QPen(QColor(164, 198, 0)));
...
void form_dataentry_robot_Dialog::GetData()
{

}

bool form_dataentry_robot_Dialog::fromStringValue(const QString &inStr, QDateTime &value) {
...
void form_dataentry_robot_Dialog::tabwidget_insert_add_column(QSqlQuery *query, QTableWidget *tab, QString nameColumnX,  int numRow, int numColumn)
{
    //QString const DateTimeFormat = "yyyy-MM-ddTHH:mm:ss[Z|[+|-]HH:mm]";
    QString const DateTimeFormat = "yyyy-MM-ddTHH:mm:ss.zzzZ";
    //QString const DateTimeFormat = "yyyy-MM-ddTHH:mm:ss.zzzZ";
...
void form_dataentry_robot_Dialog::GetData_search(QString id)
{
...
void form_dataentry_robot_Dialog::GetData_search(QString id, QString tab, QString str2)
{
    //DataSystems::Instance().log+="id: "+id+;
    logger::WriteLog(QString::number(this->id));
...
void form_dataentry_robot_Dialog::TimeOperation()
{
}

// SELECT * FROM public.dataentry_robot ORDER BY id ASC LIMIT 1

void form_dataentry_robot_Dialog::GetData_FirstId(QString id, QString tab, QString str2)
{
...
void form_dataentry_robot_Dialog::TimeOperation(QCustomPlot *customPlot)
{
...
void form_dataentry_robot_Dialog::plotRandom2()
{
...
void form_dataentry_robot_Dialog::plotRandom()
{
...
void form_dataentry_robot_Dialog::plotRandom_()
{
...
void form_dataentry_robot_Dialog::addPoint(double x, double y)
{
    cow_graph1_x.append(x);
    cow_graph1_y.append(y);
}

void form_dataentry_robot_Dialog::addPoint(double x, double y, double z)
{
    cow_graph1_x.append(x);
    cow_graph1_y.append(y);
...
void form_dataentry_robot_Dialog::addPoint(double x, double y, double z, double w)
{
    cow_graph1_x.append(x);
    cow_graph1_y.append(y);
    cow_graph2_y.append(z);
...
void form_dataentry_robot_Dialog::addPoint_x1_5(double x, double y, double z, double w)
{
    x0.append(x);
    y0.append(y);
...
void form_dataentry_robot_Dialog::clearData()
{
    //plot->clearGraphs();
...
void form_dataentry_robot_Dialog::createTableWidget()
{
    /*
...
void form_dataentry_robot_Dialog::on_pushButton_clicked()
{
...
void form_dataentry_robot_Dialog::on_pushButton_2_clicked()
{
    //mTimer->start(milliSeconds);
    //plot->clearGraphs();
    //plot->clearPlottables();
...
void form_dataentry_robot_Dialog::on_pushButton_close_clicked()
{
     this->close();
}
